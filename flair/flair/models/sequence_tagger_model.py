...
class SequenceTagger(flair.nn.Model):
    ...
    def __init__(self,
        ...
        if self.use_rnn:
            ...
            self.linear = torch.nn.Linear(hidden_size, len(tag_dictionary))
        else:
    ...
    def forward(self, sentences: List[Sentence], sort=True):
        ...
        for s_id, sentence in enumerate(sentences):
            ...
            for token in sentence:
                ...
                token_tag = self.tag_dictionary.get_idx_for_item(token.get_tag(self.tag_type).value)
        ...
        if self.use_esn:
            running_loss = 0
            running_loss += self.esn.train(sentences_and_tags)
